...
type User struct {
	Base
	Name         string         `json:"name" gorm:"column:name;type:VARCHAR(32);not null" binding:"max=10,min=3"`                                     // 姓名
	Nickname     sql.NullString `json:"nickname" gorm:"column:nickname"`                                                                              // 昵称
	Gender       int            `json:"gender" gorm:"column:gender;type:TINYINT;default:0"`                                                           // 性别
	Avatar       string         `json:"avatar" gorm:"column:avatar"`                                                                                  // 昵称
	Mobile       string         `json:"mobile" gorm:"column:mobile;type:VARCHAR(16)"`                                                                 // 手机
	Email        string         `json:"email" gorm:"column:email"`                                                                                    // 邮箱
	Homepage     string         `json:"homepage" gorm:"column:homepage"`                                                                              // 个人主页
	Birthday     time.Time      `json:"birthday" gorm:"column:birthday;type:DATE"`                                                                    // 生日
	Height       float32        `json:"height" gorm:"column:height;type:FLOAT"`                                                                       // 身高(cm)
	BloodType    string         `json:"bloodType" gorm:"column:blood_type;type:VARCHAR(8)"`                                                           // 血型(ABO)
	Notice       string         `json:"notice" gorm:"column:notice;type:TEXT"`                                                                        // 备注
	Intro        string         `json:"intro" gorm:"column:intro;type:TEXT"`                                                                          // 简介
	Address      string         `json:"address" gorm:"column:address;type:JSON"`                                                                      // 地址
	Lives        string         `json:"lives" gorm:"column:lives;type:JSON"`                                                                          // 生活轨迹
	Tags         string         `json:"tags" gorm:"column:tags;type:JSON"`                                                                            // 标签
	LuckyNumbers string         `json:"luckyNumbers" gorm:"column:lucky_numbers;type:JSON"`                                                           // 幸运数字
	Score        int            `json:"score" gorm:"column:score;default:0"`                                                                          // 积分
	Auths        []Auth         `json:"auths" gorm:"foreignkey:UserID"`                                                                               // 帐号
	Roles        []Role         `json:"roles" gorm:"many2many:userrole;"`                                                                             // 角色
	Groups       []Group        `json:"groups" gorm:"many2many:usergroup;"`                                                                           // 组
	Friends      []User         `json:"friends" gorm:"many2many:userfriend;association_jointable_foreignkey:user_id;jointable_foreignkey:friend_id;"` // 友
